...
class Shapes3D(ground_truth_data.GroundTruthData):
    ...
    def __init__(self):
        with tf.gfile.GFile(SHAPES3D_PATH, "rb") as f:
            ...
            ds_file = h5py.File(f, "r")

        images = np.array(ds_file.get("images"))
        labels = np.array(ds_file.get("labels"))
        n_samples = images.shape[0]
        self.images = images.reshape([n_samples, 64, 64, 3]).astype(np.float32) / 255.0
